...
namespace KellControls
{
	public partial class TreeControl : UserControl, ITreeEvents, ITreeInfo
	{
        ...
        void _treeState_Expanding(object sender, TreeNodeEventArgs e)
        {
            OnAfterExpand(e);
        }

        void _treeState_Collapsing(object sender, TreeNodeEventArgs e)
        {
            OnAfterCollapse(e);
        }
		...
		public KellTreeNodeCollection RootNodes
		{
			[DebuggerStepThrough]
			get
			{
		...
		public KellTreeNode SelectedNode
		{
			get
			{
		...
		{
			IDisposable id = _updatesSuspended.Apply();
            string assFullName = Assembly.GetExecutingAssembly().FullName;
            KellControls.Utility.Assemblies.ManifestResources res = new KellControls.Utility.Assemblies.ManifestResources(assFullName.Substring(0, assFullName.IndexOf(",")));//"KellControls"
            this.Icons.Clear();
            this.Icons.Add(res.GetIcon("Resources.FeedTreeItem16.ico"));
            this.Icons.Add(res.GetIcon("Resources.CollapsedFolderTreeItem16.ico"));
            this.Icons.Add(res.GetIcon("Resources.ExpandedFolderTreeItem16.ico"));
            return id;
		}
		...
		public void ExpandNode( KellTreeNode node )
		{
			if( node == null )
			{
				throw new ArgumentNullException( "node" );
		...
		public KellTreeNode GetNodeAt( Point p )
		{
			p = new Point( p.X - AutoScrollPosition.X, p.Y - AutoScrollPosition.Y );

			foreach( KellTreeNode treeNode in _verticalPositioning.GetNodesBetween( p.Y, p.Y ) )
			{
        ...
        protected virtual void OnAfterExpand(TreeNodeEventArgs e)
        {
            if (AfterExpand != null)
            {
                AfterExpand(this, e);
        ...
        protected virtual void OnAfterCollapse(TreeNodeEventArgs e)
        {
		...
		protected override void OnPaint( PaintEventArgs e )
		{
			...
			using( WinFormsUtility.Drawing.GdiPlusEx.SaveState( e.Graphics ) )
			{
				...
				KellTreeNode[] nodes = _verticalPositioning.GetNodesBetween( clipTop, clipBottom );
				...
				foreach( KellTreeNode treeNode in nodes )
				{
		...
		void ITreeEvents.NodeUpdated( KellTreeNode treeNode )
		{
		...
		{
			foreach( KellTreeNode node in _needsInvalidate )
			{
				InvalidateNode( node );
			}
        ...
        public event TreeNodeEventHandler AfterExpand;
        public event TreeNodeEventHandler AfterCollapse;
		...
		private KellTreeNodeCollection _rootNodes;
		...
		private KellTreeNode _hoverNode;
		private bool _hovering;
		private Utility.Collections.Set<KellTreeNode> _needsInvalidate = new Utility.Collections.Set<KellTreeNode>();
